const grid = [
  ...
  [32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70], // 8
  ...
  [19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 04, 89, 55, 40], // 15
...
for (let i = 0; i < grid.length; i++) {
  for (let j = 0; j < grid[i].length; j++) {
    ...
    for (let h = 0; h < 4; h++) {
      ...
      grid[i][j+h] ? horizontal += ',' + grid[i][j+h] : horizontal = 0;
      grid[i+h] ? vertical += ',' + grid[i+h][j] : vertical = 0;
      grid[i+h] && grid [i+j] ? diagonal = ',' + grid[i+h][j+h] : diagonal = 0;
      //console.log(horizontal, vertical, diagonal);

      //if (horizontal > resultado) resultado = horizontal;
      //if (vertical > resultado) resultado = vertical;
      //if (diagonal > resultado) resultado = diagonal;

      //operacion.push(horizontal);
    }
    if (horizontal !== 0) horizontal = horizontal.split(',');
    operacion.push(horizontal, vertical, diagonal);
  }
}
console.log(operacion);
//operacion = operacion.split(',').filter(num => num !== 0 && num.length > 1);
